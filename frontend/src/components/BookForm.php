      ...
      foreach ($this->book_item_deltas as $delta) {
        ...
        if (strpos($delta, '_new') === FALSE) {
          ...
          if (!empty($book_item_entity)) {
            ...
            $form['book_item']['old']['container'][$delta]['book_id_label_' . $delta] = array(
             '#markup' => '<h3>' . $link . '</h3>',
            );
            $form['book_item']['old']['container'][$delta]['book_id'] = array(
              ...
              foreach ($this->book_item_note_deltas[$delta] as $note_id) {
                ...
                if (strpos($note_id, '_new') === FALSE) {
                  ...
                  $form['book_item']['old']['container'][$delta]['book_notes']['new'][$note_id]['note_' . $note_id . '_' . $delta] = [
                    '#type' => 'textarea',
                    '#attributes' => array('placeholder' => $this->t('Note')),
          ...
          $form['book_item']['new']['container'][$delta] = array(
            '#prefix' => '<hr /><div class="row">',
            '#suffix' => '</div>',
          );
          $form['book_item']['new']['container'][$delta]['book_id_' . $delta] = array(
            '#type' => 'textfield',
            '#required' => TRUE,
          ...
          $form['book_item']['new']['container'][$delta]['condition_' . $delta] = array (
            '#type' => 'select',
            '#options' => $this->service->getTaxonomyIdFromVid('conditions'),
            '#required' => TRUE,
            '#description' => $this->t('This can only be changed if the book is Checked In.'),
          ...
          $form['book_item']['new']['container'][$delta]['checked_status'] = array(
          ...
          $form['book_item']['new']['container'][$delta]['remove'] = array (
            '#type' => 'submit',
            '#value' => $this->t('remove'),
